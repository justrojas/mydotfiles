...
# --- inshellisense: IDE-style autocomplete (auto-start) --------------------
# Wraps interactive shells in an inshellisense session. init.zsh has its own
# guards (ISTERM / non-interactive / VSCode); placed before oh-my-zsh so the
# outer shell hands off early and the inner session re-sources the full rc.
# Escape hatch: `export INSHELLISENSE_DISABLE=1` to fall back to on-demand `is`
# (e.g. if it conflicts with a multiplexer/agent). Generated by terminal-setup.sh.
if [[ -z "$INSHELLISENSE_DISABLE" && -f ~/.inshellisense/init/zsh/init.zsh ]]; then
