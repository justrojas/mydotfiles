...
# Wraps interactive shells in an inshellisense session. init.bash has its own
# guards (ISTERM / non-interactive / VSCode). Escape hatch:
# `export INSHELLISENSE_DISABLE=1` to fall back to on-demand `is`.
# Generated by terminal-setup.sh (`is init bash`).
if [ -z "$INSHELLISENSE_DISABLE" ] && [ -f ~/.inshellisense/init/bash/init.sh ]; then
